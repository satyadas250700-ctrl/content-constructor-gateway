...
async function getAccessToken() {
  ...
  tokenPromise = (async () => {
    try {
      ...
      /*
       * GigaChat token живёт около 30 минут.
...
async function askGigaChatInternal(
  ...
  try {
    ...
    /*
     * 429:
     * не создаём параллельные запросы.
     * Повторяем через 3, затем 6 секунд.
     */
    if (
      status === 429 &&
      retryAttempt < 2
...
Это не полный сценарий каждого материала.
Каждый день должен давать понятную,
практичную и конкретную идею публикации.
...
Но идея каждого дня должна быть достаточно
понятной, чтобы человек мог сразу понять,
что именно создавать и публиковать.
...
==================================================
ПРАВИЛО «НЕ ПРИДУМЫВАЙ АТРИБУТЫ БИЗНЕСА»
==================================================

Даже если атрибут кажется очевидным
для данного бизнеса,
не считай его автоматически фактом.

Например, если пользователь сообщил
только «кофейня», нельзя утверждать,
...
- Wi-Fi;
- розетки;
- интерьер определённого типа;
- терраса;
- посадочные места;
- кухня;
- выпечка собственного производства.

Если это не было сообщено пользователем,
не утверждай это как существующий атрибут.


==================================================
ИСТОЧНИК КОНКРЕТНЫХ СУЩНОСТЕЙ
==================================================

Разрешённые конкретные сущности =
то, что прямо присутствует
в информации пользователя.

Если конкретное существительное
отсутствует в данных пользователя,
не используй его как утверждение
о товаре, услуге или характеристике бизнеса.


==================================================
НО ТВОРЧЕСТВО НЕ ОГРАНИЧИВАЙ
==================================================

Очень важно:

НЕ превращай контент-план
в сухой и безликий список.
...
==================================================
РАЗНИЦА МЕЖДУ ФАКТОМ И СЦЕНАРИЕМ
==================================================

Можно предложить:

«Снимите сцену, где человек делает
первый глоток и мир вокруг будто
замедляется».

Нельзя писать:

«Наш бариста каждое утро
готовит кофе на новейшей кофемашине»,
...
«Представьте день, в котором
десерт становится маленьким
праздником посреди рутины».

Нельзя писать:

«Наши десерты готовятся
из фермерских продуктов»,
...
СЦЕНАРИЙ / ОБРАЗ — можно придумывать.

ФАКТ О БИЗНЕСЕ — нельзя придумывать.


==================================================
НЕ ДОБАВЛЯЙ СЛУЧАЙНЫЕ КОНКРЕТНЫЕ ДЕТАЛИ
==================================================

Если бизнес описан общими словами,
оставайся на соответствующем уровне конкретики.
...
Но нельзя без разрешения
переходить к конкретным блюдам,
ингредиентам или технологиям.
...
практичным;
эмоциональным;
пригодным для публикации.

Не повторяй одну и ту же мысль
разными словами.

Не делай каждый день продажей.

Не используй одинаковый CTA
пять раз подряд.

Не начинай каждый день
одинаковым способом.

Следи за тем,
чтобы соседние дни отличались
по задаче и формату.


==================================================
ВАЖНО ДЛЯ ПОСЛЕДОВАТЕЛЬНОСТИ 30 ДНЕЙ
==================================================
...
Не повторяй один и тот же формат
несколько раз подряд без причины.

Продолжай общую логику контент-плана.

Каждый новый блок должен ощущаться
как продолжение предыдущего,
а не как отдельный случайный список.
      ...
      [
        {
          role: "system",
          content:
            "Ты профессиональный русскоязычный контент-стратег. Создавай практичные и креативные контент-планы, строго соблюдая фактические данные пользователя.",
        },
        {
          role: "user",
          content: prompt,
